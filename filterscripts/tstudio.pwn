...
#define EditCheck(%0); if(CurrObject[%0] == -1) { \
	...
	return SendClientMessage(playerid, STEALTH_YELLOW, "Você precisa selecionar um objeto para usar este comando"); }

// Don't let player use command if they are editing an object
#define NoEditingMode(%0); if(EditingMode[%0]) { \
	SendClientMessage(playerid, STEALTH_ORANGE, "______________________________________________"); \
	return SendClientMessage(playerid, STEALTH_YELLOW, "Você precisa terminar de editar seu objeto antes de usar este comando"); }

// Checks if a map is open
#define MapOpenCheck(); if(!MapOpen) { \
	SendClientMessage(playerid, STEALTH_ORANGE, "______________________________________________"); \
	return SendClientMessage(playerid, STEALTH_YELLOW, "Não há nenhum mapa aberto no momento"); }
