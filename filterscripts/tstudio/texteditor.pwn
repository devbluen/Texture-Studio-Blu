...
YCMD:text(playerid, arg[], help)
{
	if(help)
	{
		...
		SendClientMessage(playerid, STEALTH_GREEN, "Adicione/edite/remova texto em um objeto. (Apenas o slot de textura 0 é suportado.)");
		return 1;
	}
	...
	SendClientMessage(playerid, STEALTH_ORANGE, "______________________________________________");
	SendClientMessage(playerid, STEALTH_GREEN, "Começou a editar o texto");
...
UpdateTextEditor(playerid)
{
	...
	else PlayerTextDrawSetString(playerid, Click_SetText[playerid], "Clique para editar");
	...
	// Update alignment
	if(GetObjectAlignment(index) == 0) PlayerTextDrawSetString(playerid, Click_Alignment[playerid], "Esquerda");
	else if(GetObjectAlignment(index) == 1) PlayerTextDrawSetString(playerid, Click_Alignment[playerid], "Centro");
	else if(GetObjectAlignment(index) == 2) PlayerTextDrawSetString(playerid, Click_Alignment[playerid], "Direita");
...
public ClickPlayerTextDrawEditText(playerid, PlayerText:playertextid)
{
	...
	else if(playertextid == Click_SetText[playerid])
	{
			...
			if(response)
			{
				if(strlen(text) > 0)
				{
					if(strlen(text) < MAX_TEXT_LENGTH)
					{
						...
						PlayerTextDrawSetString(playerid, Click_SetText[playerid], "Clique para editar");
					...
					{
					    SendClientMessage(playerid, STEALTH_ORANGE, "______________________________________________");
					    SendClientMessage(playerid, STEALTH_YELLOW, "O comprimento do texto é muito longo");
					}
				}
				else
				{
				    SendClientMessage(playerid, STEALTH_ORANGE, "______________________________________________");
				    SendClientMessage(playerid, STEALTH_YELLOW, "O texto deve conter pelo menos 1 caractere.");
				}
                return 1;
			}
		}
		...
		Dialog_ShowCallback(playerid, using inline Response, DIALOG_STYLE_INPUT, "Definir texto material", tmptext, "Ok", "Cancelar");
		return 1;
	}
 	...
 	else if(playertextid == Click_FontTextSize[playerid])
	{
		inline Response(pid, dialogid, response, listitem, string:text[])
		{
			...
			if(response)
			{
				...
				{
				    SendClientMessage(playerid, STEALTH_ORANGE, "______________________________________________");
				    SendClientMessage(playerid, STEALTH_YELLOW, "Tamanho de fonte de texto inválido (0 - 200).");
				}
			}
		}
		Dialog_ShowCallback(playerid, using inline Response, DIALOG_STYLE_INPUT, "Definir tamanho do texto da fonte", "Escolha um tamanho de texto de fonte", "Ok", "Cancelar");
		return 1;
	}
	...
	else if(playertextid == Click_FontColor[playerid])
	{
        ...
        Dialog_ShowCallback(playerid, using inline Response, DIALOG_STYLE_INPUT, "Definir cor da fonte", "Insira um valor hexadecimal para a cor da fonte (ARGB) ", "Ok", "Cancelar");
		return 1;
	}
    ...
    else if(playertextid == Click_BackColor[playerid])
    {
        ...
        Dialog_ShowCallback(playerid, using inline Response, DIALOG_STYLE_INPUT, "Definir cor traseira", "Insira um valor hexadecimal para a cor da fonte (ARGB) ", "Ok", "Cancelar");
		return 1;
    }
...
{
	TextEdit_Background_0 = TextDrawCreate(10.000000, 150.000000, "Texto On:");
	...
	TextEdit_Background_2 = TextDrawCreate(10.000000, 160.000000, "Texto:");
	...
	TextEdit_Background_3 = TextDrawCreate(10.000000, 170.000000, "Fonte:");
	...
	TextEdit_Background_4 = TextDrawCreate(10.000000, 180.000000, "Tamanho da fonte:");
	...
	TextEdit_Background_5 = TextDrawCreate(10.000000, 190.000000, "Fonte em negrito:");
	...
	TextEdit_Background_6 = TextDrawCreate(10.000000, 200.000000, "Cor da fonte:");
	...
	TextEdit_Background_8 = TextDrawCreate(10.000000, 222.000000, "Alinhamento:");
	...
	TextEdit_Background_9 = TextDrawCreate(10.000000, 233.000000, "Tamanho do texto da fonte:");
