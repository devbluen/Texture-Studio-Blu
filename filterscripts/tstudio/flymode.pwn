...
YCMD:flymode(playerid, arg[], help)
{
	if(help)
	{
		...
		SendClientMessage(playerid, STEALTH_GREEN, "Alternar modo de voo.");
		return 1;
	}
...
YCMD:fmspeed(playerid, arg[], help)
{
	...
	{
		SendClientMessage(playerid, STEALTH_ORANGE, "______________________________________________");
		SendClientMessage(playerid, STEALTH_GREEN, "Altere a velocidade do flymode, deve ser 5-200.");
		return 1;
	}
    ...
    SendClientMessage(playerid, STEALTH_GREEN, sprintf("Velocidade máxima do Flymode definida como% 0,2f", newspeed));
	return 1;
}
...
YCMD:fmaccel(playerid, arg[], help)
{
	...
	{
		SendClientMessage(playerid, STEALTH_ORANGE, "______________________________________________");
		SendClientMessage(playerid, STEALTH_GREEN, "Alterar a taxa de aceleração do flymode, deve ser 0,005 - 0,05");
		return 1;
	}
    ...
    SendClientMessage(playerid, STEALTH_GREEN, sprintf("Fvelocidade máxima do flymode definida como% 0,3f", newacc));
	return 1;
}
...
YCMD:fmtoggle(playerid, arg[], help)
{
	...
	{
		SendClientMessage(playerid, STEALTH_ORANGE, "______________________________________________");
		SendClientMessage(playerid, STEALTH_GREEN, "Alternar aceleração do modo fly.");
		return 1;
	}

	noclipdata[playerid][accel] = !noclipdata[playerid][accel];
	SendClientMessage(playerid, STEALTH_GREEN, sprintf("Aceleração do Flymode alternada %s", noclipdata[playerid][accel] ? ("on") : ("off")));
	return 1;
}
