    ...
    else if(clickedid == ListSelSelectObject)
	{
		if(Iter_Contains(Objects, CurrListOffset[playerid]+CurrListHighlight[playerid]))
		{
            if(SetCurrObject(playerid, CurrListOffset[playerid]+CurrListHighlight[playerid])) {
                new line[128];
                format(line, sizeof(line), "Você selecionou o índice do objeto %i para edição", CurrListOffset[playerid]+CurrListHighlight[playerid]);
		...
		else
		{
			...
			SendClientMessage(playerid, STEALTH_YELLOW, "Esse objeto não existe!");
		}
	}
...
YCMD:lsel(playerid, arg[], help)
{
	...
	{
		SendClientMessage(playerid, STEALTH_ORANGE, "______________________________________________");
		SendClientMessage(playerid, STEALTH_GREEN, "Exibe uma lista de todos os objetos atualmente carregados para seleção.");
		return 1;
	}
