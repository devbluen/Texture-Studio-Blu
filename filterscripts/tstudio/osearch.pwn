...
YCMD:osearch(playerid, arg[], help)
{
	if(help)
	{
		...
		SendClientMessage(playerid, STEALTH_GREEN, "Pesquise um modelo de objeto por palavra-chave.");
		return 1;
	}
	...
	for(new i; i < sizeof(ObjectList); i++)
	{
        if(strfind(ObjectList[i][oName],arg, true) != -1)
		{
	        ...
	        format(line, sizeof(line), "Nome do objeto: %s ID do modelo: %i", ObjectList[i][oName],ObjectList[i][oID]);
        ...
        {
            SendClientMessage(playerid, STEALTH_YELLOW, "Quantidade máxima de objetos encontrados!");
            break;
        }
	}
	...
	{
		SendClientMessage(playerid, STEALTH_ORANGE, "______________________________________________");
		SendClientMessage(playerid, STEALTH_YELLOW, "Nenhum objeto encontrado, tente pesquisar novamente");
	}
	else
	{
		format(line, sizeof(line), "Total de objetos encontrados: %i", totalobjectsfound);
...
YCMD:osearchex(playerid, arg[], help)
{
	...
	{
		SendClientMessage(playerid, STEALTH_ORANGE, "______________________________________________");
		SendClientMessage(playerid, STEALTH_GREEN, "Procure um modelo de objeto por expressão.");
		return 1;
	}
...
static UpdateOSearchPage(playerid)
{
	...
	for(new i = 0; i < MAX_OS_PAGE; i++)
	{
		...
		{
			format(line, sizeof(line), "~r~ID:~g~ %i ~r~Nome:~g~ %s",
		...
		else PlayerTextDrawSetString(playerid, OSearchIndex[playerid][i], "~r~ID:~g~ -1 ~r~Nome:~g~ Nenhum");
	}
	UpdateOSPreview(playerid);
	return 1;
}
...
static CreatePlayerSearchDraw(playerid)
{
	...
	for(new i = 0; i < MAX_OS_PAGE; i++)
	{
		OSearchIndex[playerid][i] = CreatePlayerTextDraw(playerid,20.000000, y, "~r~ID:~g~ 1337 ~r~Nome:~g~ sinal_01");
