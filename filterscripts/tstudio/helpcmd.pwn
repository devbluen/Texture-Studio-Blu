...
new Commands[][COMMAND_INFO] = {
    ...
    {1, 0, "cobject (curto: /co)"},
    {1, 0, "dobject (curto: /do)"},
    {1, 0, "robject"},
    {1, 0, "clone"},
    {1, 0, "ogroup"},
    {1, 0, "ogoto"},
...
YCMD:thelp(playerid, arg[], help)
{
	if(help)
	{
		...
		SendClientMessage(playerid, STEALTH_GREEN, "Veja uma lista de todos os comandos e veja o que eles fazem.");
		return 1;
	}
		...
		if(sresponse)
		{
			...
			inline SelectCommand(epid, edialogid, eresponse, elistitem, string:etext[])
			{
				...
				if(eresponse)
				{
					...
					Dialog_ShowCallback(playerid, using inline SelectCommand, DIALOG_STYLE_LIST, "Texture Studio - Lista de Comandos", cmds, "Selecionar", "Back");
				}
				else
					Dialog_ShowCallback(playerid, using inline SelectCommandType, DIALOG_STYLE_LIST, "Texture Studio - Lista de Comandos", cmdtypes, "Selecionar", "Cancelar");
			}
			Dialog_ShowCallback(playerid, using inline SelectCommand, DIALOG_STYLE_LIST, "Texture Studio - Lista de Comandos", cmds, "Selecionar", "Back");
		}
	}
