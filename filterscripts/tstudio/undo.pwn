...
YCMD:undo(playerid, arg[], help)
{
	if(help)
	{
		...
		SendClientMessage(playerid, STEALTH_GREEN, "Desfaça a última ação.");
		return 1;
	}

    MapOpenCheck();
    NoEditingMode(playerid);
	if(UndoLastAction()) SendClientMessage(playerid, STEALTH_GREEN, "A última ação foi desfeita.");
	else SendClientMessage(playerid, STEALTH_YELLOW, "Não há ações para desfazer.");
	return 1;
}
