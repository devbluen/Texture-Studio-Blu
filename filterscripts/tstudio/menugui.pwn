...
// Sub-Menus ///////////////////////////////////////////////////////////////////
	...
	GUISetPlayerText(SelectionSubMenu, E_INDEX[1], "Selecionar");
...
OnGUIClick:ObjectMenu(playerid, group, gindex, pindex)
{
	...
	switch(group)
	{
		...
		case CLICK_SEARCH_MENU:
		{
	        ...
	        Dialog_ShowCallback(playerid, using inline SearchObject, DIALOG_STYLE_INPUT, "Texture Studio", "Input object search string", "Ok", "Cancelar");
		}
		...
		case CLICK_MOVEMENT_MENU:
		{
			...
			else if(ObjectMovementIndex[13] == pindex)
			{
                ...
                Dialog_ShowCallback(playerid, using inline SetMovementInc, DIALOG_STYLE_INPUT, "Texture Studio", "Input object movement inc", "Ok", "Cancelar");
			}
			...
			else if(ObjectMovementIndex[14] == pindex)
			{
                ...
                Dialog_ShowCallback(playerid, using inline SetMovementRot, DIALOG_STYLE_INPUT, "Texture Studio", "Input object rotation inc", "Ok", "Cancelar");
			}
		}
	}
	return 1;
...
OnGUIClick:SubObjectEdit(playerid, group, gindex, pindex)
{
	...
	switch(group)
	{
	    ...
	    case CLICK_OBJECTEDIT_COBJECT:
		{
			...
			Dialog_ShowCallback(playerid, using inline ChooseObject, DIALOG_STYLE_INPUT, "Texture Studio", "Input objectid to create", "Ok", "Cancelar");
		}
		...
		case CLICK_OBJECTEDIT_OGROUP:
		{
			...
			Dialog_ShowCallback(playerid, using inline GroupAssign, DIALOG_STYLE_INPUT, "Texture Studio", "Input group assignment for object", "Ok", "Cancelar");
		}
	}
...
OnGUIClick:SubMenuSel(playerid, group, gindex, pindex)
{
	...
	switch(group)
	{
	    ...
	    case CLICK_SELECT_OBJECT:
		{
			...
			Dialog_ShowCallback(playerid, using inline ChooseSelection, DIALOG_STYLE_INPUT, "Texture Studio", "Input object selection ID", "Ok", "Cancelar");
		}
...
OnGUIClick:SubMenuTex(playerid, group, gindex, pindex)
{
	...
	switch(group)
	{
		...
		case CLICK_TEXTURE_SEARCH:
		{
			...
			Dialog_ShowCallback(playerid, using inline TextureSearch, DIALOG_STYLE_INPUT, "Texture Studio", "Search for a texture name", "Ok", "Cancelar");
		}
	}
	return 1;
}
	...
	switch(group)
	{
		...
		case CLICK_MOVEMENTG_MENU:
		{
			...
			else if(ObjectGroupMovementIndex[13] == pindex)
			{
                ...
                Dialog_ShowCallback(playerid, using inline SetMovementGInc, DIALOG_STYLE_INPUT, "Texture Studio", "Input group movement inc", "Ok", "Cancelar");
			}
			...
			else if(ObjectGroupMovementIndex[14] == pindex)
			{
                ...
                Dialog_ShowCallback(playerid, using inline SetMovementGRot, DIALOG_STYLE_INPUT, "Texture Studio", "Input group rotation inc", "Ok", "Cancelar");
			}
		}
	}
	return 1;
...
OnGUIClick:SubMenuGroupSel(playerid, group, gindex, pindex)
{
	switch(group)
	{
		...
		case CLICK_GROUP_GADD:
		{
			...
			Dialog_ShowCallback(playerid, using inline AddGroupObject, DIALOG_STYLE_INPUT, "Texture Studio", "Input object to add to group", "Ok", "Cancelar");
		}
		...
		case CLICK_GROUP_GREM:
		{
			...
			Dialog_ShowCallback(playerid, using inline RemGroupObject, DIALOG_STYLE_INPUT, "Texture Studio", "Input object to remove from group", "Ok", "Cancelar");
		}
		...
		case CLICK_GROUP_SETGROUP:
		{
			...
			Dialog_ShowCallback(playerid, using inline SetGroup, DIALOG_STYLE_INPUT, "Texture Studio", "Set grouped objects to group", "Ok", "Cancelar");
		}
		...
		case CLICK_GROUP_SELECTGROUP:
		{
			...
			Dialog_ShowCallback(playerid, using inline SelectGroup, DIALOG_STYLE_INPUT, "Texture Studio", "Input group to select", "Ok", "Cancelar");
		}
		...
		case CLICK_GROUP_GDELETE:
		{
			...
			Dialog_ShowCallback(playerid, using inline ConfirmGDelete, DIALOG_STYLE_MSGBOX, "Texture Studio", "Are you sure you want to delete all objects in this group?", "Ok", "Cancelar");
		}
	...
	switch(group)
	{
		...
		case CLICK_GROUP_LOADPF:
		{
			...
			else
			{
				...
				Dialog_ShowCallback(playerid, using inline SelectPrefab, DIALOG_STYLE_LIST, "Texture Studio - Load Prefab", list, "Ok", "Cancelar");
			}
		}
			...
			else
			{
				inline SelectLoadZ(pid, dialogid, response, listitem, string:text[])
				{
					...
					if(response)
					{
						...
						Dialog_ShowCallback(playerid, using inline ChooseZ, DIALOG_STYLE_INPUT, "Texture Studio - Set Prefab Load Z", "Enter the load Z-Offset for this prefab", "Ok", "Cancelar");
					}
				}
				Dialog_ShowCallback(playerid, using inline SelectLoadZ, DIALOG_STYLE_LIST, "Texture Studio - Load Prefab", list, "Ok", "Cancelar");
			}
		}
	}
	return 1;
					...
					if(olistitem == 0)
					{
				   		inline SetBindName(spid, sdialogid, sresponse, slistitem, string:stext[])
					   	{
							...
							if(sresponse)
							{
								...
								Dialog_ShowCallback(playerid, using inline ChooseEditOption, DIALOG_STYLE_LIST, "Texture Studio - Choose Edit Type", "Edit Bind Name\nEdit Bind Text\nDelete Bind", "Ok", "Cancelar");
							}
							Dialog_ShowCallback(playerid, using inline ChooseEditOption, DIALOG_STYLE_LIST, "Texture Studio - Choose Edit Type", "Edit Bind Name\nEdit Bind Text\nDelete Bind", "Ok", "Cancelar");
						}
						Dialog_ShowCallback(playerid, using inline SetBindName, DIALOG_STYLE_INPUT, "Texture Studio - Edit Bind Text", "Set Bind Name", "Ok", "Cancelar");
				            ...
				            inline EditLine(epid, edialogid, eresponse, elistitem, string:etext[])
							{
								...
								if(eresponse)
								{
					   				inline SetBind(spid, sdialogid, sresponse, slistitem, string:stext[])
								   	{
										...
										Dialog_ShowCallback(playerid, using inline EditLine, DIALOG_STYLE_LIST, "Texture Studio - Edit Bind Text", line, "Ok", "Cancelar");
									}
									Dialog_ShowCallback(playerid, using inline SetBind, DIALOG_STYLE_INPUT, "Texture Studio - Edit Bind Text", "Set Bind", "Ok", "Cancelar");
								}
							}
							format(line, sizeof(line), "");
						    for(new i = 0; i < MAX_BINDS_PER_BIND; i++) { format(line, sizeof(line), "%s%i: %s\n", line, i, CommandBindData[clistitem][i]); }
					...
					else if(olistitem == 2)
					{
			            inline DeleteBind(dpid, dialogid, dresponse, dlistitem, string:dtext[])
						{
                            ...
                            if(dresponse)
                            {
								...
								Dialog_ShowCallback(playerid, using inline ChooseBind, DIALOG_STYLE_LIST, "Texture Studio - Edit Binds", line, "Ok", "Cancelar");
							}
						...
						Dialog_ShowCallback(playerid, using inline DeleteBind, DIALOG_STYLE_MSGBOX, "Texture Studio - Delete", "Delete this bind?", "Ok", "Cancelar");
					}
				}
			}
			Dialog_ShowCallback(playerid, using inline ChooseEditOption, DIALOG_STYLE_LIST, "Texture Studio - Choose Edit Type", "Edit Bind Name\nEdit Bind Text\nDelete Bind", "Ok", "Cancelar");
